...
export default {
  ...
  methods: {
    submitForm() {
      if(this.ruleForm.oldpass === this.$session.get("userpassword")){
        const _this = this;
        axios.put("http://localhost:8282/user/updateUserPwd/" 
           + _this.ruleForm.newpass +"&"+this.$session.get("useraccount")
         ).then(function(res) {
            console.log(res);
            if(res.data == 'success'){
              alert("密码修改成功！")
